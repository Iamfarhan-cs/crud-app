...
## Build Phase G: Database Migrations

Build Phase G defines the PostgreSQL migration files for the task table.
...
The up migration creates the `tasks` table if it does not already exist.
...
UUID values are generated by the Go application before inserts reach PostgreSQL. The migration does not create a database-side UUID default.

The title constraint uses `length(trim(title)) > 0` so empty and whitespace-only titles cannot be stored.

The status constraint allows only these lifecycle values:

- `pending`
- `in_progress`
- `done`

`deleted_at` supports soft delete behavior so normal reads can exclude deleted tasks while preserving historical rows.

### Active Task Index

The migration creates the partial index `idx_tasks_active_created_at`:
...
This supports active task listing in newest-first order without indexing soft-deleted rows for the common list path.

### Down Migration

The down migration drops `idx_tasks_active_created_at` if it exists, then drops the `tasks` table if it exists.

### Build Phase G Non-Goals

Build Phase G does not include:

- A migration runner.
- Go application code changes.
- Repository behavior changes.
- Handler or service changes.
- Database connection wiring changes.
